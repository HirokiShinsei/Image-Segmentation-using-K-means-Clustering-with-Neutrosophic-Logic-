...
def neutrosophic_indeterminacy(mean_image, std_image, diff_image):
    denominator = np.maximum(std_image, diff_image)
    indeterminacy_image = np.divide((std_image * diff_image), np.where(denominator == 0, 1, denominator), out=np.zeros_like(denominator), where=denominator!=0)
...
def neutrosophic_clustering(truth_image, falsity_image, indeterminacy_image, k):
    if k == 1:
        # If k is 1, return a black image
        return np.zeros_like(truth_image, dtype=np.uint8)
...
def image_segmentation(image_path, k, min_cluster_size):
    ...
    if k == 1:
        # If k is 1, visualize the indeterminacy as an overlay on the original image
        indeterminacy_overlay = np.zeros_like(image_np)
        indeterminacy_overlay[:, :, 2] = indeterminacy_image * 255  # Set red channel based on indeterminacy

        overlaid_image = cv2.addWeighted(image_np, 1, indeterminacy_overlay, 0.7, 0)
        ...
        plt.title("Indeterminacy Overlay")
    else:
...
if __name__ == "__main__":
    image_path = "2nd_year/Intro to AI/midterm/Lisa.jpg"  # Path to the image to be segmented
    k = 2  # Number of clusters
    min_cluster_size = 160  # Minimum cluster size
